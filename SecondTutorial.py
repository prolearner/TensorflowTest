...
# and the last is the number of output channels. We will also have a bias vector with a component
...
#In order to build a deep network, we stack several layers of this type. The second layer
...
#  that a neuron's output is kept during dropout. This allows us to turn dropout on during training,
...
# How well does this model do? To train and evaluate it we will use code that is nearly identical to that
# for the simple one layer SoftMax network above. The differences are that: we will replace the steepest
# gradient descent optimizer with the more sophisticated ADAM optimizer; we will include the additional
# parameter keep_prob in feed_dict to control the dropout rate; and we will add logging to every 100th
